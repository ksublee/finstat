...
조건부 분산이 $\hat\sigma_N^2 = 4 \times 10^{-6}$으로 더 컸다면 $\mathrm{VaR}_{0.01}$은 대략 \$102.5가 된다. 현재의 변동성이 클수록 앞으로의 위험도 크게 추정된다는 뜻이다.
:::
...
r <- 0.002; X_N <- 10000
v <- omega / (1 - alpha - beta)  # sigma^2

for (i in 1:100000){
  X <- X_N
  for (j in 2:6){
    v[j] <- omega + alpha * r[j-1]^2 + beta * v[j-1]
    r[j] <- sqrt(v[j]) * rnorm(1)
    X <- X * (1 + r[j])
  }
  Xs[i] <- X
}
